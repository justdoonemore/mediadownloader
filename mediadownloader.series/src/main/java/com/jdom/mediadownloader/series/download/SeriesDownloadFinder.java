...
public class SeriesDownloadFinder {
	 ...
	 */
	public SeriesDownloadFinder(Series series,
			SeriesLinkFinder seriesLinkFinder, String[] titleExclusions) {
		this.series = series;
	...
	/**
	 * Checks all links available for this series, finding any possible
	 * downloads.
	 * 
	 * @param seriesLinkFinder
	...
	public Collection<SeriesDownload> findDownloads() {
		...
		Collection<String> possibleLinks = seriesLinkFinder
				.getPossibleLinksForSeries(series);
		...
		for (String link : possibleLinks) {
			...
			if (seriesDownload != null) {
				...
				if (matchingLinks.contains(seriesDownload)
						&& LOG.isDebugEnabled()) {
					LOG.debug("Skipping link, " + seriesDownload
							+ " already set to be downloaded...");
				} else {
					if (LOG.isDebugEnabled()) {
						LOG.debug("Adding link, " + seriesDownload
								+ " not already set to be downloaded...");
					}
	...
	private SeriesDownload checkForMatchingLink(String link) {
		...
		SeriesDownloadLink seriesDownloadLink = new RssSeriesDownloadLink(
				series, link, titleExclusions);

		boolean processLink = seriesDownloadLink
				.matchesSeriesDownloadCriteria();
		...
		if (processLink) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Found season [" + seriesToDownload.getSeason()
						+ "] episode [" + seriesToDownload.getEpisode()
						+ "] for Series [" + series.getName() + "]");
			}
