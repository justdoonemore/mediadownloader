...
public class Series extends AbstractEntity<Series> {
	...
	public int compareTo(Series o) {
		...
		if (compareVal == 0) {
			...
			int thatSeason = o.getSeason();
			if (thisSeason > thatSeason) {
				compareVal = 1;
			} else if (thatSeason > thisSeason) {
				compareVal = -1;
			}

			if (compareVal == 0) {
				int thisEpisode = this.getEpisode();
				int thatEpisode = o.getEpisode();
				if (thisEpisode > thatEpisode) {
					compareVal = 1;
				} else if (thatEpisode > thisEpisode) {
					compareVal = -1;
				}
			}
		}
